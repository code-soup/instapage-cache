 ...
 *
 * @package CodeSoup\InstapageCache\Traits
 */
		...
		if ( ! self::is_wp_version_ok( $config['MIN_WP_VERSION'] ) ) {
			self::deactivate_plugin(
				...
				sprintf(
					...
					esc_html__( 'Minimum WordPress version required: %1$s. You are running version: %2$s.', 'instapage-cache' ),
		...
		if ( ! self::is_php_version_ok( $config['MIN_PHP_VERSION'] ) ) {
			self::deactivate_plugin(
				...
				sprintf(
					...
					esc_html__( 'Minimum PHP version required: %1$s. You are running version: %2$s.', 'instapage-cache' ),
